...
function* onAddNewJobList({ payload: data }) {
    try {
        ...
        toast.success("لیست مشاغل با موفقیت اضافه شد", { autoClose: 2000 });
    } catch (error) {
        yield put(addJobListFail(error))
        toast.error("فهرست کار اضافه نشد", { autoClose: 2000 });
    }
}
...
function* onUpdateJobList({ payload: data }) {
    try {
        ...
        toast.success("لیست مشاغل با موفقیت به روز شد", { autoClose: 2000 });
    } catch (error) {
        yield put(updateJobListFail(error))
        toast.error("لیست مشاغل به روز نشد", { autoClose: 2000 });
    }
}
...
function* onDeleteJobList({ payload: data }) {
    ...
    try {
        ...
        toast.success("لیست مشاغل با موفقیت حذف شد", { autoClose: 2000 });
    } catch (error) {
        yield put(deleteJobListFail(error))
        toast.error("فهرست کار حذف نشد", { autoClose: 2000 });
    }
}
...
function* OnDeleteApplyJob({ payload: data }) {
    try {
        ...
        toast.success("کار کاربردی با موفقیت حذف شد", { autoClose: 2000 });
    } catch (error) {
        yield put(deleteApplyJobFail(error))
        toast.error("کار اعمال شده حذف شد ناموفق", { autoClose: 2000 });
    }
}
